...
_spec = importlib.util.find_spec(MOD_NAME)
if _spec is None:
    pytest.skip("skill_space not found. Place it next to this test.", allow_module_level=True)
...
def test_end_to_end_two_cluster_segmentation():
    ...
    params = make_params(K=2, D=2, alpha=2.0, kappa=6.0, gamma=1.0)
    ...
    mu_part1 = m0 + torch.randn(T//2, D, dtype=torch.float64) * 0.1
    mu_part2 = m1 + torch.randn(T - T//2, D, dtype=torch.float64) * 0.1
    mu_t = torch.cat([mu_part1, mu_part2], dim=0)
    diag_var = torch.full((T, D), cov, dtype=torch.float64)

    # Run a few VI updates
    for _ in range(8):
        out = model.update(mu_t, diag_var, mask=torch.ones(T, dtype=torch.float64), rho=1.0, n_e_steps=1, optimize_pi=True)
    ...
    assert acc >= 0.75, f"Segmentation accuracy too low: {acc:.3f}"
